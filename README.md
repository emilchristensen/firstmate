...
| `/updatefirstmate` | Self-update the running firstmate and its secondmates to the latest from origin with fast-forward-only pulls, then re-read instructions and nudge secondmates; also detects when the fork is behind its upstream repo and opens a reviewable upstream merge PR (never a blind merge into main) |
