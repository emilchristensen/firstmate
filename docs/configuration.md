...
It emits `SECONDMATE_SYNC:` only when a home was skipped for an actionable sync reason or config inheritance failed.
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|Running [0-9]+ shell command|\(([0-9]+m )?[0-9]+s [^)]*[0-9]+(\.[0-9]+)?k tokens'   # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper; owner FM_TMUX_BUSY_REGEX_DEFAULT in bin/fm-tmux-lib.sh, see docs/tmux-backend.md "Claude busy signatures"
